...
layui.config({
    ...
    //监听工具条
    table.on('tool(cleanList)',function (obj) {
        var data=obj.data;
        if(obj.event=='del')
        {
            ...
            layer.confirm('真的删除吗？',function (index) {
                $.ajax({
                    url:ctx+"/clean/delClean/"+data.id,
                    type:"get",
                    success:function (d) {
                        if(d.code==0)
                        {
                            table.reload('cleanList',{});//重新发送请求 表格重载
                        }
                        else
                        {
        ...
        else if(obj.event=='edit')
        {
            layer.open({
                ...
                title:"编辑角色",
                area:['380px','600px'],
                content:ctx+"/clean/editClean/"+data.id
            })
        }
    });
    //添加角色
    $(".cleanAdd_btn").click(function () {
        var index=layer.open({
            title:"添加打扫记录",
            type:2,
            content:ctx+"/clean/addClean",
            area:['380px','550px']
        });
